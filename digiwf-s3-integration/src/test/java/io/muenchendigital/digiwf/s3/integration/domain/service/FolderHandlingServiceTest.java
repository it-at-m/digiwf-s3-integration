...
class FolderHandlingServiceTest {
    ...
    void deleteFolderException() throws S3AccessException {
        ...
        Mockito.when(this.fileRepository.findByPathToFileStartingWith(pathToFolder)).thenReturn(Stream.of(file));
        ...
        Mockito.when(this.fileRepository.findByPathToFileStartingWith(pathToFolder)).thenReturn(Stream.empty());
        Assertions.assertThrows(S3AndDatabaseAsyncException.class, () -> this.folderHandlingService.deleteFolder(pathToFolder));
